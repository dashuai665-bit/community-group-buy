export function formatMoney(minor:number,currency='TWD'){const amount=minor/100;if(currency==='TWD')return `NT$${new Intl.NumberFormat('zh-TW',{minimumFractionDigits:minor%100===0?0:2,maximumFractionDigits:2}).format(amount)}`;return new Intl.NumberFormat('zh-TW',{style:'currency',currency,maximumFractionDigits:0}).format(amount);}
