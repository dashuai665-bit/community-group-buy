...
export interface OrderingPolicy {
  requireVerifiedPhoneForOrder?: boolean;
}

export function isProfileComplete(
  profile: Pick<ProfileRow, 'display_name' | 'phone'>,
): boolean {
  return Boolean(profile.display_name?.trim() && profile.phone?.trim());
}

export function canProceedToOrdering(
  profile: Pick<ProfileRow, 'display_name' | 'phone' | 'phone_verified'>,
  policy: OrderingPolicy = {},
): boolean {
  if (!isProfileComplete(profile)) return false;
  return (
    !policy.requireVerifiedPhoneForOrder || profile.phone_verified === 'true'
  );
}
...
export class IdentityService {
  ...
  async resolveAppUser(
    identity: AuthenticatedProviderIdentity,
  ): Promise<string> {
    if (!identity.verified || !identity.providerUserId.trim()) {
      throw new ApiError(401, 'UNVERIFIED_IDENTITY', '登入身份尚未驗證');
    }
    const existing = await this.repositories.identities.find(
      identity.provider,
    ...
    try {
      ...
      const winner = await this.repositories.identities.find(
        identity.provider,
        identity.providerUserId,
      );
  ...
  async linkIdentityToAuthenticatedUser(
    userId: string | null,
    identity: AuthenticatedProviderIdentity,
  ): Promise<void> {
    const actor = await requireActiveUser(this.repositories, userId);
    if (!identity.verified || !identity.providerUserId.trim())
      throw new ApiError(422, 'INVALID_IDENTITY', '登入身份資料無效');
    const existing = await this.repositories.identities.find(
      identity.provider,
      identity.providerUserId,
    );
    ...
    if (existing)
      throw new ApiError(409, 'IDENTITY_OWNED', '此登入身份已綁定其他會員');
    await this.repositories.batch([
      this.repositories.identities.insertStatement(
        createId(),
        actor.id,
  ...
  async unlinkIdentity(
    ...
    if (!identity || identity.user_id !== actor.id)
      throw new ApiError(403, 'IDENTITY_NOT_OWNED', '無權解除此登入身份');
    if ((await this.repositories.identities.countForUser(actor.id)) <= 1)
      throw new ApiError(409, 'LAST_IDENTITY', '不可解除最後一個登入身份');
    await this.repositories.batch([
      this.repositories.identities.deleteStatement(identityId),
      this.repositories.audits.insertStatement({
        id: createId(),
        actorUserId: actor.id,
        actionType: 'identity_unlink',
        targetType: 'user_identity',
        targetId: identityId,
      }),
    ]);
...
export async function requireActiveUser(
  ...
  if (user.status !== 'active')
    throw new ApiError(403, 'USER_INACTIVE', '會員帳號目前不可執行此操作');
  return user;
}
...
export class CommunityMembershipService {
  ...
  async join(userId: string | null, communityId: string) {
    ...
    if (!community || community.status !== 'active')
      throw new ApiError(404, 'COMMUNITY_NOT_FOUND', '找不到可加入的社區');
    const existing = await this.repositories.members.find(
      actor.id,
      communityId,
    );
    ...
    if (community.join_policy === 'approval_required')
      throw new ApiError(409, 'APPROVAL_REQUIRED', '此社區需要管理員審核');
    if (community.join_policy === 'invite_only')
      throw new ApiError(403, 'INVITE_REQUIRED', '此社區僅限受邀會員');
    const membershipId = existing?.id ?? createId();
    await this.repositories.batch([
      existing
        ? this.repositories.members.activateStatement(actor.id, communityId)
        : this.repositories.members.insertStatement(
            membershipId,
            actor.id,
            communityId,
          ),
    ...
    return {
      membership: await this.repositories.members.find(actor.id, communityId),
      created: true,
    };
  }

  async assertCanLeaveCommunity(
    userId: string,
    communityId: string,
  ): Promise<void> {
    ...
    if (!membership || membership.status !== 'active')
      throw new ApiError(404, 'MEMBERSHIP_NOT_FOUND', '找不到有效 membership');
    if (
      membership.role === 'community_admin' &&
      (await this.repositories.members.countActiveAdmins(communityId)) <= 1
    ) {
      throw new ApiError(
        409,
        'LAST_COMMUNITY_ADMIN',
        '請先移交管理權再離開社區',
      );
    }
    if (
      await this.repositories.orders.hasUnfinishedForCommunity(
        userId,
        communityId,
      )
    ) {
  ...
  async leave(userId: string | null, communityId: string): Promise<void> {
    ...
    await this.repositories.batch([
      ...
      this.repositories.profiles.reassignDefaultStatement(
        actor.id,
        communityId,
      ),
      this.repositories.audits.insertStatement({
...
export class CommunityPreferenceService {
  ...
  async setDefault(userId: string | null, communityId: string): Promise<void> {
    ...
    if (!membership || membership.status !== 'active')
      throw new ApiError(422, 'ACTIVE_MEMBERSHIP_REQUIRED', '必須先加入該社區');
    await this.repositories.batch([
      ...
      this.repositories.audits.insertStatement({
        ...
        actorUserId: actor.id,
        communityId,
        actionType: 'default_community_change',
...
export class CommunityAdminService {
  ...
  async listManageableCommunities(actorUserId: string | null) {
    ...
    if (await this.repositories.platformRoles.isPlatformAdmin(actor.id)) {
      return {
        ...
        communities: await this.repositories.communities.listAll(),
      };
    }
    const memberships = (await this.repositories.members.listManagedForUser(
      actor.id,
  ...
  async getMemberContact(
    ...
    const platformAdmin = await this.repositories.platformRoles.isPlatformAdmin(
      actor.id,
    );
    const membership = await this.repositories.members.find(
      actor.id,
    ...
    if (
      !platformAdmin &&
      (membership?.status !== 'active' || membership.role !== 'community_admin')
    ) {
      throw new ApiError(
        403,
    ...
    const contact = await this.repositories.members.findActiveTargetContact(
      targetUserId,
      communityId,
    );
    if (!contact)
      throw new ApiError(404, 'MEMBER_NOT_FOUND', '找不到該社區會員');
    await this.repositories.batch([
      this.repositories.audits.insertStatement({
        id: createId(),
        actorUserId: actor.id,
        communityId,
        actionType: 'admin_member_contact_access',
        targetType: 'user',
        targetId: targetUserId,
        metadata: sanitizeLogFields({
...
export class ProfileService {
  ...
  async changeDisplayName(
    ...
    const actor = await requireActiveUser(this.repositories, userId);
    await this.repositories.batch([
      this.repositories.profiles.changeDisplayNameStatement(
        actor.id,
        displayName,
      ),
    ]);
