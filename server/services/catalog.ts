...
export class ProductCatalogService {
  ...
  async create(
    ...
    if (!(await this.repositories.platformRoles.isPlatformAdmin(actor.id)))
      throw new ApiError(403, 'PLATFORM_ADMIN_REQUIRED', '需要平台管理員權限');
    const id = createId();
...
export class CommunityOfferingService {
  ...
  async create(
    ...
    const actor = await requireCommunityManager(
      this.repositories,
      actorUserId,
      communityId,
    );
    ...
    if (!product || product.status !== 'active')
      throw new ApiError(422, 'PRODUCT_INACTIVE', '商品不存在或未啟用');
    const id = createId();
    try {
      await this.repositories.batch([
        this.repositories.offerings.insertStatement({
          id,
          communityId,
          ...input,
        }),
        this.repositories.audits.insertStatement({
          ...
          actorUserId: actor.id,
          communityId,
          actionType: 'offering_created',
          targetType: 'community_product_offering',
          targetId: id,
        }),
      ]);
    } catch {
  ...
  async update(
    ...
    if (!offering || offering.community_id !== communityId)
      throw new ApiError(404, 'OFFERING_NOT_FOUND', '找不到此社區 offering');
    await this.repositories.batch([
...
export class GroupBuyBatchService {
  ...
  async findOrCreateOpenBatch(offeringId: string) {
    ...
    if (!offering || offering.status !== 'active')
      throw new ApiError(422, 'OFFERING_INACTIVE', 'offering 目前不可湊單');
    const batches = await this.repositories.batches.listForOffering(offeringId);
    const sequence = (batches.at(-1)?.sequence_number ?? 0) + 1;
    try {
      await this.repositories.batch([
        this.repositories.batches.createOpenStatement(
          createId(),
          offeringId,
          sequence,
          offering.batch_threshold,
        ),
      ]);
    } catch {
  ...
  async commitQuantity(
    ...
    if (
      !offering ||
      offering.community_id !== communityId ||
      offering.status !== 'active'
    )
      throw new ApiError(404, 'OFFERING_NOT_FOUND', '找不到可湊單 offering');
    if (
      quantity < offering.min_quantity_per_order ||
      (offering.max_quantity_per_order !== null &&
        quantity > offering.max_quantity_per_order)
    ) {
      throw new ApiError(
        422,
  ...
  async openNextBatch(offeringId: string) {
    return this.findOrCreateOpenBatch(offeringId);
  }
  async formBatchWhenThresholdReached(offeringId: string) {
    return this.repositories.batches.listForOffering(offeringId);
  }
}
...
export class ProductWishService {
  ...
  async create(
    actorUserId: string | null,
    communityId: string,
    input: { productId?: string | null; wishText?: string | null },
  ) {
    const actor = await requireActiveUser(this.repositories, actorUserId);
    const community = await this.repositories.communities.findById(communityId);
    if (!community || community.status !== 'active')
      throw new ApiError(422, 'COMMUNITY_INACTIVE', '社區目前不可新增願望');
    const membership = await this.repositories.members.find(
      actor.id,
      communityId,
    );
    ...
    if (!input.productId && !input.wishText?.trim())
      throw new ApiError(
        422,
        'WISH_CONTENT_REQUIRED',
        'productId 或 wishText 至少需要一項',
      );
    const id = createId();
    await this.repositories.batch([
      this.repositories.wishes.insertStatement({
        id,
        userId: actor.id,
        communityId,
        ...input,
      ...
      this.repositories.audits.insertStatement({
        ...
        actorUserId: actor.id,
        communityId,
        actionType: 'wish_created',
        targetType: 'product_wish',
        targetId: id,
      }),
    ]);
    return this.repositories.wishes.findById(id);
  }
  async listMine(actorUserId: string | null) {
    const actor = await requireActiveUser(this.repositories, actorUserId);
    return this.repositories.wishes.listForUser(actor.id);
  }
  async listCommunity(actorUserId: string | null, communityId: string) {
    await requireCommunityManager(this.repositories, actorUserId, communityId);
    return this.repositories.wishes.listForCommunity(communityId);
  }
  async updateStatus(
    actorUserId: string | null,
    communityId: string,
    ...
    if (!wish || wish.community_id !== communityId)
      throw new ApiError(404, 'WISH_NOT_FOUND', '找不到此社區願望');
    await this.repositories.batch([
      ...
      this.repositories.audits.insertStatement({
        ...
        actorUserId: actor.id,
        communityId,
        actionType: 'wish_status_changed',
        targetType: 'product_wish',
        targetId: wishId,
        metadata: { from: wish.status, to: status },
      }),
    ]);
